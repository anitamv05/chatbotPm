...
if user_query:
    ...
    if st.button("Summarize Selected Posts"):
        if selected_posts:
        # Summarization logic using the new OpenAI client structure
            summaries = []
        for title in selected_posts:
            # Modify the prompt to request a more detailed summary
            summary_prompt = f"Provide a detailed and informative summary of the blog post titled: {title}. Include key points, insights, and actionable advice from the post."
            ...
            summary_completion = client.chat.completions.create(
                ...
                max_tokens=300  # Increased max tokens for more detailed summaries
            )
            summaries.append(f"Summary of {title}: {summary_completion.choices[0].message.content.strip()}")
